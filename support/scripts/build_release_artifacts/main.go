...
func main() {
	...
	pkg := packageName(bin)
	repo := "github.com/humaniq/atticlab-go"
...
func build(pkg, dest, version, buildOS, buildArch string) {
	...
	timeFlag := fmt.Sprintf("-X github.com/humaniq/atticlab-go/support/app.buildTime=%s", buildTime)
	versionFlag := fmt.Sprintf("-X github.com/humaniq/atticlab-go/support/app.version=%s", version)
